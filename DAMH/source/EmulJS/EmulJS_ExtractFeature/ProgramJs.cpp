...
void CProgramJs::SetData(LPVOID pBuffHtmlCode, DWORD dwSizeBuff)
{
	...
	PreProcess(m_pFileHtmlProcess->GetCodeHtml());
...
CVarLink *CProgramJs::FunctionCall(bool &bExecute, CVarLink *pVLFunc, CVar *pVParent, bool bExecuteInEval)
{
	...
	char sFuncName[64] = { 0 };
	strcpy(sFuncName, pVLFunc->m_sName.c_str());
	CheckFunctionCall(sFuncName, sFuncName);
...
{
	CRuntimeException *pExceptionVr = NULL;	// Exception Neu co Virus

	CTokenPointer* pTokenParent = m_pTokenPointer;	// Con tro lenh Chuong trinh cha
	CTokenPointer* pTokenParent2 = NULL;
	CTokenPointer *pTokenCond = NULL;	// Con tro lenh cho dk vong lap
	CTokenPointer *pTokenIter = NULL;	// Con tro lenh cho bien dem vong lap
	CTokenPointer *pTokenBody = NULL;	// Con tro lenh cho than vong lap
	...
	bool bResultRunStatement = true;	// Bien ket qua thuc hien lenh
	bool bCondIf;
...
bool EmulJS(SCANRESULT* pResult, LPVOID pBuffDataHtml, DWORD dwSizeBuf)
{
	...
	try
	{
		...
		if (nCountSzCodeJs > 0)
		{
			// FIX::: Save luc load module. + ko giai phong
			// Lưu giá trị kết quả trả về vào thành phần result
			//
			//	result -> m_sName: "result"
			//		   -> CVar m_pVar: "[return value]"
			//
			//
			//
			//
			...
			for (i = 0; i < nCountSzCodeJs; i++)
			{
				...
				sCodeJs = lstsCodeJs.at(i); // đoạn code thứ i
				...
				try 
				{
					...
					if (sCodeJs.length() > 0)
					{
						...
						g_pProgramJs->SaveToFile("data_sach" + to_string(g_pProgramJs->countt), sCodeJs);
						g_pProgramJs->ExportCSV("data_sach" + to_string(g_pProgramJs->countt) + ".csv", "0");//dir
						g_pProgramJs->ResetFeature();
						g_pProgramJs->countt += 1;
					}
				...
				catch (CRuntimeException *pRE)
				{
					...
					g_pProgramJs->SaveToFile("data_sach" + to_string(g_pProgramJs->countt), sCodeJs);
					g_pProgramJs->ExportCSV("data_sach" + to_string(g_pProgramJs->countt) + ".csv", "0");
					g_pProgramJs->ResetFeature();
					g_pProgramJs->countt += 1;
					nErrId = pRE->GetErrId();
					if (nErrId == EXCEPTIONID_FOUND_VIRUS
				...
				catch (int e)
				{
					...
					{
						// Luu thong tin ra file
						g_pProgramJs->SaveToFile("data_outmem" + to_string(g_pProgramJs->countt), sCodeJs);
						g_pProgramJs->ExportCSV("data_outmem" + to_string(g_pProgramJs->countt) + ".csv", "0");
						g_pProgramJs->ResetFeature();
						g_pProgramJs->countt += 1;
...
// 				or catch(...) to catch all exceptions
// 								catch (invalid_argument&){
// 									OutputDebug("Loi: Tham so dau vao ham stoi()!");
// 								}
// 				
// 								// Loi tran bo nho 
// 								catch (out_of_range&){
// 									OutputDebug("Loi: Tran bo nho()!");
// 								}
// 				
// 								// Loi khong xu ly duoc => nguy co mem leak
// 								catch (...){
// 									OutputDebug("_LOI Crash: exception C++");
// 								}
